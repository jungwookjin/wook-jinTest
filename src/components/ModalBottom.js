...
export default class ModalBottom extends React.Component {
    render() {
        const { _modalCb, isModalOpen } = this.props;
        ...
        return (
            <Modal
                ...
                <View style={{ width: Layout.window.width - 40, marginBottom: 6, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.9)', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
                    <TouchableOpacity onPress={() => { _modalCb(true, { isCamera: false }) }} style={{ width: '100%', height: 56, justifyContent: 'center', alignItems: 'center' }}>
                        <Text allowFontScaling={false} style={{ fontSize: Layout.fsM, color: 'blue' }}>사진 앨범</Text>
                    </TouchableOpacity>

                    <View style={{ width: '96%', height: 1, backgroundColor: 'rgba(0,0,0,0.1)' }}></View>

                    <TouchableOpacity onPress={() => { _modalCb(true, { isCamera: true }) }} style={{ width: '100%', height: 56, justifyContent: 'center', alignItems: 'center' }}>
                        <Text allowFontScaling={false} style={{ fontSize: Layout.fsM, color: 'blue' }}>카메라</Text>
                    </TouchableOpacity>
                </View>
